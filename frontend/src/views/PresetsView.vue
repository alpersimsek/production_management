...
const currentPresetId = ref(null) // Store presetId for rule modal
...
onMounted(async () => {
  ...
  try {
    ...
    error.value = 'Failed to load data: ' + (err.message || err || 'Unknown error')
  } finally {
    loading.value = false
  }
})
...
const handlePresetSaved = async () => {
  try {
    ...
    error.value = `Failed to refresh preset list: ${err.message || err || 'Unknown error'}`
  }
}
...
const handleRuleSaved = async (presetId) => {
  try {
    ...
    error.value = `Failed to refresh rules list: ${err.message || err || 'Unknown error'}`
  }
}
...
const handleDelete = async () => {
  try {
    ...
    error.value = `Failed to delete ${deleteType.value}: ${err.message || err || 'Unknown error'}`
  } finally {
    loading.value = false
  }
}
    ...
    <div class="min-h-screen bg-gray-50 px-4 sm:px-6 lg:px-8 py-8">
      <div class="sm:flex sm:items-center mb-8">
        <div class="sm:flex-auto">
          ...
          <p class="mt-2 text-sm text-gray-600 font-medium">Seamlessly manage product presets and their associated rules
          </p>
        </div>
