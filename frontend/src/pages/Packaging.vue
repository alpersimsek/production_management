              ...
              <div v-if="filteredJobs.length > 0" class="space-y-3">
                ...
                <div class="flex items-center justify-between">
                  <div class="text-sm text-gray-500">
                    {{ $t('packaging.showing_job') }} {{ currentJobIndex + 1 }} {{ $t('common.of') }} {{ filteredJobs.length }}
                  </div>
                ...
                <div class="text-center text-xs text-gray-400">{{ $t('packaging.swipe_hint') }}</div>
                ...
                >
                  <div class="flex items-center justify-between mb-2">
                    ...
                    <span :class="getStatusColor(currentJob.status)" class="inline-flex px-2 py-1 text-xs font-semibold rounded-full">{{ $t('packaging.status.' + currentJob.status) }}</span>
                  </div>
                  <div class="grid grid-cols-1 gap-2 text-sm">
                    <div class="flex justify-between"><span class="text-gray-500">{{ $t('packaging.product') }}:</span><span class="text-gray-900">{{ currentJob.product_name }}</span></div>
                    <div class="flex justify-between"><span class="text-gray-500">{{ $t('packaging.lot') }}:</span><span class="text-gray-900">{{ currentJob.lot_number || '-' }}</span></div>
                    <div class="flex justify-between"><span class="text-gray-500">{{ $t('packaging.package_type') }}:</span><span class="text-gray-900">{{ currentJob.package_type }}</span></div>
                    <div class="flex justify-between"><span class="text-gray-500">{{ $t('common.quantity') }}:</span><span class="text-gray-900">{{ currentJob.quantity_packaged }} {{ currentJob.unit }}</span></div>
                    <div class="flex justify-between"><span class="text-gray-500">{{ $t('packaging.packaging_date') }}:</span><span class="text-gray-900">{{ currentJob.packaging_date }}</span></div>
                  </div>
                  <div class="flex justify-end space-x-2 mt-3 pt-3 border-top border-gray-100">
...
function nextJob () { if (currentJobIndex.value < filteredJobs.value.length - 1) currentJobIndex.value++ }
function previousJob () { if (currentJobIndex.value > 0) currentJobIndex.value-- }
function handleJobTouchStart (e) { const t = e.touches[0]; touchStartX.value = t.clientX; touchStartY.value = t.clientY; isDragging.value = false }
function handleJobTouchMove (e) { if (!touchStartX.value && !touchStartY.value) return; const t = e.touches[0]; const dx = t.clientX - touchStartX.value; const dy = t.clientY - touchStartY.value; if (Math.abs(dx) > Math.abs(dy)) { isDragging.value = true; e.preventDefault() } }
function handleJobTouchEnd (e) { if (!isDragging.value) return; const t = e.changedTouches[0]; const dx = t.clientX - touchStartX.value; if (Math.abs(dx) > 50) { if (dx > 0) previousJob(); else nextJob() } touchStartX.value = 0; touchStartY.value = 0; isDragging.value = false }

// Mouse drag parity
function handleJobMouseDown (e) { touchStartX.value = e.clientX; touchStartY.value = e.clientY; isDragging.value = false }
function handleJobMouseMove (e) { if (!touchStartX.value && !touchStartY.value) return; const dx = e.clientX - touchStartX.value; const dy = e.clientY - touchStartY.value; if (Math.abs(dx) > Math.abs(dy)) isDragging.value = true }
function handleJobMouseUp (e) { if (!isDragging.value) return; const dx = e.clientX - touchStartX.value; if (Math.abs(dx) > 50) { if (dx > 0) previousJob(); else nextJob() } touchStartX.value = 0; touchStartY.value = 0; isDragging.value = false }
...
function handleKeydown (event) {
  if (event.key === 'Escape') {
    if (showAddModal.value || showEditModal.value || showDetailsModal.value) closeModal()
  }
}
